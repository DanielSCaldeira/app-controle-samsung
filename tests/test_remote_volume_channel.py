"""Tests for task 30103d06 â€” Controles de volume, mute e canal (RemoteScreen).

BotÃµes VOL+/VOL-/MUTE e CH+/CH- adicionados Ã  tela de controle, ligados ao
RemoteViewModel.

Acceptance criteria verificado aqui:
  **Teste de UI: cada botÃ£o envia o RemoteKey correto (KEY_VOLUP, KEY_VOLDOWN,
...
elo crÃ­tico exigido pelo aceite: para CADA um dos 5 novos controles, a tela
...
em vez de sÃ³ inspecionar fontes â€” ver ``test_remote_screen.py`` /
...
harness percorre os 5 novos controles, resolve cada tecla no catÃ¡logo e a roteia
...
# --------------------------------------------------------------------------- #
# Harness â€” reproduces what the volume/channel row does for every new control:
...
def out(tmp_path_factory):
    ...
    assert cr.returncode == 0, (
        "compilaÃ§Ã£o do harness volume/canal falhou:\n" + (cr.stdout or "") + (cr.stderr or "")
    )
...
# --------------------------------------------------------------------------- #
# Acceptance â€” each NEW button routes to the correct intent/frame (with fake).
# --------------------------------------------------------------------------- #
...
def test_all_five_controls_emit_distinct_frames(out):
    ...
    # No control is hard-coded to another's key â€” 5 controls, 5 distinct frames.
