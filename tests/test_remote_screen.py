...
# The nine controls the screen exposes, paired with the protocol code each must
# emit. This is the contract under test: control -> KEY_* -> intent.
CONTROLS = [
    ("DPAD_UP", "KEY_UP"),
    ("DPAD_DOWN", "KEY_DOWN"),
    ("DPAD_LEFT", "KEY_LEFT"),
...
// The exact codes RemoteScreen resolves from the catalog, one per control.
...
fun main() {
    ...
    for (code in CONTROL_CODES) {
        ...
        // then the resolved key is pressed through the ViewModel.
...
def out(tmp_path_factory):
    ...
    tmp = tmp_path_factory.mktemp("remote_screen")
    src = tmp / "src"
...
def test_screen_is_stateless_and_emits_press_key_intents():
    text = _read(SCREEN_KT)
    assert "onIntent: (RemoteIntent) -> Unit" in text, (
        "a tela deve ser stateless e expor um callback onIntent(RemoteIntent)"
    )
    assert "RemoteIntent.PressKey" in text, "cada controle deve emitir RemoteIntent.PressKey"


def test_screen_defines_test_tags_for_every_control():
    text = _read(SCREEN_KT)
...
def test_screen_enforces_min_touch_target_of_48dp():
    text = _read(SCREEN_KT)
    # A single named minimum is applied to every control via sizeIn.
...
def test_route_observes_viewmodel_and_forwards_intents():
    text = _read(SCREEN_KT)
    assert "RemoteViewModel" in text, "a rota deve usar o RemoteViewModel"
    assert "viewModel.onIntent" in text, "a rota deve encaminhar os intents ao ViewModel"
...
def test_compose_ui_test_authored():
    ...
    # Touch-target assertions at runtime.
    assert "assertWidthIsAtLeast(48.dp)" in text and "assertHeightIsAtLeast(48.dp)" in text, (
        "teste de UI deve verificar alvos de toque >= 48 dp"
    )
